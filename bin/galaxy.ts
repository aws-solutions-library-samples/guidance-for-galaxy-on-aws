...
const providerStack = new ProviderStack(app, 'Provider', {
  env: { account, region },
  description: 'EKS Cluster Stack - Guidance for Galaxy on AWS (SO9346)',
});

const galaxyInfraStack = new InfrastructureStack(app, 'GlxInfra', {
  env: { account, region },
  description:
    'Infrastructure DB,RDS,MQ Stack - Guidance for Galaxy on AWS (SO9346)',
...
const applicationStack = new ApplicationStack(app, 'GlxApp', {
  env: { account, region },
  description:
    'Galaxy  Application Stack - Guidance for Galaxy on AWS (SO9346)',
  eksCluster: providerStack.eksCluster,
  ...
  visit: (node: IConstruct) => {
    if (node instanceof lambda.CfnFunction) {
      node.addPropertyOverride(
        'Environment.Variables.AWS_STS_REGIONAL_ENDPOINTS',
        'regional'
      );
    }
  },
