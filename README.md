...
| vpc.enableFlowlogs             | No                               | false                                       | If no vpc.id is provided and a new VPC created, this toggle enables/disables VPC flow logs for the newly created VPC. No changes to existing VPC are performed.
...
| galaxy.loadBalancerCertArn     | No                               | arn:aws:acm:us-east-1:122333:certificate/ce | ARN of a certificate in Amazon Certificate Manager. Enables TLS/SSL communication with the load balancer fronting Galaxy                                                                                                                                                                                                               |
| galaxy.loadbalancerAccessLogsBucketname | No                      | galaxy-alb-access-log-bucket                | If a S3 bucketname is provided the Loadbalancer sends the data to it; the bucket requires a compliant bucket policy (with prefixes "galaxy" and "tusd" https://docs.aws.amazon.com/elasticloadbalancing/latest/application/enable-access-logging.html)
